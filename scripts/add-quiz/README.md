...
* [ゴールまでの流れ](#ゴールまでの流れ)
* [0. ウォーミングアップ: スクリプトが何をやるかを理解しよう](#0-ウォーミングアップ-スクリプトが何をやるかを理解しよう)
* [1. 既存ノートにクイズ用フィールドを追加する](#1-既存ノートにクイズ用フィールドを追加する)
* [2. txt として エクスポートする(txt-1)](#2-txt-として-エクスポートするtxt-1)
* [3. スクリプトで txt-1 を処理しクイズを追加した新しい txt-2 を作る](#3-スクリプトで-txt-1-を処理しクイズを追加した新しい-txt-2-を作る)
* [4. 新しくできたファイルを(txt-2) を Anki にインポートする。](#4-新しくできたファイルをtxt-2-を-anki-にインポートする)
...
0. ウォーミングアップ: スクリプトが何をやるかを理解しよう
1. 既存ノートにクイズ用フィールドを追加する
2. テキストファイル形式(txt) でエクスポートする(txt-1)
3. スクリプトで txt-1 にクイズを追加し、新しい txt-2を作る
4. txt-2 を　Ankiに インポート
5. 新しいカードタイプを作って、クイズ用のコード、CSSを追加

# 0. ウォーミングアップ: スクリプトが何をやるかを理解しよう

サンプルファイルを用意したので、触りながら理解してみよう。

- [sample-notes.txt](https://github.com/t9md/awesome-tadoku/tree/master/scripts/add-quiz) は Anki からエクスポートしたNotesのテキストファイルのイメージ
- `りんご` の横の予め用意した空フィールドにクイズを挿入するのがミッションだ。
...
フィールド4にクイズが挿入された。デフォルトで選択肢は 4つだ。`-n 3`として選択肢を３つにしたりもできる。
...
# 2. txt として エクスポートする(txt-1)

<p><img src="../../imgs/anki/quiz-2.png" width="400"></p>

# 3. スクリプトで txt-1 を処理しクイズを追加した新しい txt-2 を作る
...
# 4. 新しくできたファイルを(txt-2) を Anki にインポートする。
